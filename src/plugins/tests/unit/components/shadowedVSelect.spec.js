...
describe('Test ShadowedVSelect component', () => {
    ...
    const optionsDictDefaultLabel = [{ label: 'A' }, { label: 'B' }, { label: 'C' }]
    ...
    it('has the options set from array of dict with non default label name', async () => {
        const optionsDictChangedLabel = [{ element: 'A' }, { element: 'B' }, { element: 'C' }]
        const newlabel = 'element'
        await wrapper.setProps({ options: optionsDictChangedLabel, label: newlabel })
        const allOptions = wrapper.findAll('option')
